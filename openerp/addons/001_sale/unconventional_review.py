...
#        'name':lambda self,cr,uid,c: self.pool.get('ir.sequence').get(cr,uid,'unconventional.review'),
    ...
    def create(self,cr,uid,vals,context=None):
          ...
          if vals.get('name','/')=='/':
                vals['name'] = self.pool.get('ir.sequence').get(cr, uid, 'unconventional.review') or '/'
          id=super(unconventional_review,self).create(cr,uid,vals,context=context)
          return id 
    ...
    def wait(self,cr,uid,ids,context):
        ...
        if info.state=='w_eng':
           ...
           co=line_obj.browse(cr,uid,se)
           re=co.review_date
           print co.consume_time,'co.consume_time'
           if co.consume_time:
               total_time=co.consume_time+consume_time
           if re==False:
        ...
        if info.state not in['w_eng','draft']:
            ...
            lines=line_obj.search(cr,uid,[('unconventional_review_id','=',info.id),('state','=','draft')])
            line_infoes=line_obj.browse(cr,uid,min(lines))
            if line_infoes.return_date:
                return_time=round((now-datetime.datetime.strptime(line_infoes.return_date,'%Y-%m-%d %H:%M:%S')).seconds/3600.0,2)+(now-datetime.datetime.strptime(line_infoes.return_date,'%Y-%m-%d %H:%M:%S')).days*24
            
            if line_infoes.consume_time:
                print line_info.consume_time,'line_info.consume_time'
                print return_time,'return_time'
                total_time=line_infoes.consume_time+return_time
        ...
        if info.state=='w_tech':
            ...
            print co.consume_time,'co.consume_time'
            if co.consume_time:
                total_time=co.consume_time+return_time
            if re==False:
    ...
    def button_refuse(self,cr,uid,ids,context=None):
        info=self.browse(cr,uid,ids[0])
        if info.state=='done':
            raise  osv.except_osv(_('Warning !'), _('完成状态下不允许作废！'))
