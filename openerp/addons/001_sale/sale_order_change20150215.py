...
class sale_order_change(osv.osv):
    ...
    _columns={
        ...
        'pcb_info_id':fields.many2one('pcb.info',u'用户单号',required=True),
...
#        self.change_state(cr,uid,ids)
#        self.write(cr,uid,ids,{'state':state})
#        if state=='done':
#            self.renew_state(cr,uid,ids)
#        return True
...
#        for record in my.ref_obj:
#            record=record.ref_obj
...
#        for record in my.so_change_lines_ids:
#            record=record.ref_obj
#            obj=self.pool.get(record._table_name)
#            temp_state=record.temp_state
#            if temp_state=='wait_change':
#                continue
#            else:
        record=my.ref_obj
        obj=self.pool.get(record._table_name)
    ...
    def write_dpt(self,cr,uid,ids,context=None):
        ...
        s=[u'计划部',u'工艺部',u'物控部',u'采购',u'品质部']
        so={u'计划部':'wait_plan',
               u'工艺部':'wait_tech',
               u'物控部':'wait_stock',
               u'采购':'wait_purchase',
       ...
       ###存档资料更改
        ...
        if my.state not in ['draft','wait_plan_clerk','wait_eng','done','wait_order_manager','wait_order_center']:
            ...
            if lines.state !='done':
                dpt=obj.browse(cr,uid,line_ids[0]).dpt_id.name
                print dpt
                if len(line_ids)>1:
        ...
        if my.state=='wait_order_manager' and my.type=='company_change' and my.change_company !=True and my.change_sale_order_new !=True:
            pcb_info.write(cr,uid,my.pcb_info_id.id,{'state':'draft'})
            self.write(cr,uid,ids[0],{'state':'done'})
        ...
        if my.state=='wait_order_manager' and my.type=='online_board_change':
            ...
            if my.change_sale_order_new==True:
                    ...
                    try:
                        ...
                        cur=conn.cursor()  
                        sql=''' exec PP_contract_OE '%s',%f,'','','True','',''  ''' %(my.pcb_info_id.order_recive_id.name,my.loss_amount)
                        print sql,'sql'
                        cur.execute(sql)  
                    except:
                ...
                if my.change_pcb_info_id==True:
                        pcb_info.write(cr,uid,my.pcb_info_id.id,{'state':'wait_change'})
                        try:
                            conn=pymssql.connect(server=server,user=user,password=password,database=database)  
                            cur=conn.cursor()  
                            sql=''' exec PP_contract_OE '%s',0,'','','','True',''  ''' %(my.pcb_info_id.order_recive_id.name)
                            print sql,'sql'
                            cur.execute(sql)
                        except:
                            raise osv.except_osv(_('Error!'),_(u'更新失败，请检查数据是否正确！'))
...
class so_change_records(osv.osv):
    ...
    _columns={
        ...
        'dpt_id':fields.many2one('res.department',u'部门'),
        'responsible_name':fields.many2one('res.users',u'审批人'),
